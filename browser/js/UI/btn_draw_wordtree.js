...
module.exports = exports = $('#keyword-tree-control-div #btn-draw-wordtree').click(function(){
  ...
  co(function*(){
    ...
    .use_lemma(use_lemma).root_word(text).year(year).to_year(to_year).direction(DIRECTION.FORWARD);
    var load_reverse = LoadWordTree().type(type).field(field).level(level).use_stopwords(use_stopwords)
    .use_lemma(use_lemma).root_word(text).year(year).to_year(to_year).direction(DIRECTION.REVERSE);
    $(loading).show();
